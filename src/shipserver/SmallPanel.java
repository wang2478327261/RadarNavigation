...
public class SmallPanel extends JPanel implements Runnable {  //船舶绘制有点问题，下一个版本制作的时候应当注意
    ...
    private double mousex, mousey;  //鼠标活动的坐标
    private double dragx, dragy;  //拖动时的坐标
    private double oldx, oldy;  //按下之后记录的坐标
    private double newx, newy;  //松开之后的新坐标
    private double delx, dely;  //删除船舶时点击的坐标
    private String type = "Normal";  //这个默认，不能更改，以后的版本中再加入这个功能

    String helpStr = "";
    String nameStr = "", positionStr = "", courseStr = "", speedStr = "", typeStr = "";
    private boolean pressed = false;
    ServerThread server;  //服务端通信

    private List<Ship> clientShips = new LinkedList<Ship>();  //接受客户端信号创建的对象
